...
def init_db():
    with conn() as c:
        ...
        CREATE TABLE IF NOT EXISTS employees (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, national_id TEXT, phone TEXT, role TEXT, department TEXT, hire_date TEXT, salary INTEGER DEFAULT 0, status TEXT DEFAULT 'على رأس العمل', skills TEXT, notes TEXT);
        ...
        CREATE TABLE IF NOT EXISTS contracts (id INTEGER PRIMARY KEY AUTOINCREMENT, building_id INTEGER, contract_no TEXT UNIQUE, contract_type TEXT, value INTEGER DEFAULT 0, services_included TEXT, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'ساري', notes TEXT, FOREIGN KEY(building_id) REFERENCES buildings(id) ON DELETE SET NULL);
        CREATE TABLE IF NOT EXISTS materials (id INTEGER PRIMARY KEY AUTOINCREMENT, item_code TEXT UNIQUE, barcode_sku TEXT, arabic_name TEXT NOT NULL, english_name TEXT, category TEXT, unit TEXT, quantity INTEGER DEFAULT 0, min_quantity INTEGER DEFAULT 0, reorder_point INTEGER DEFAULT 0, purchase_price INTEGER DEFAULT 0, avg_cost INTEGER DEFAULT 0, supplier TEXT, storage_location TEXT, shelf_no TEXT, manufacturer TEXT, part_no TEXT, model TEXT, serial_no TEXT, warranty_date TEXT, notes TEXT);
        CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, ticket_no TEXT UNIQUE, building_id INTEGER, asset_id INTEGER, location TEXT, room TEXT, system_type TEXT, job_type TEXT, priority TEXT, description TEXT, technician_id INTEGER, supervisor_id INTEGER, report_date TEXT, assignment_date TEXT, start_date TEXT, completion_date TEXT, sla_hours INTEGER DEFAULT 24, estimated_cost INTEGER DEFAULT 0, actual_cost INTEGER DEFAULT 0, status TEXT DEFAULT 'جديد', root_cause TEXT, corrective_action TEXT, safety_required INTEGER DEFAULT 0, materials_used TEXT, notes TEXT, FOREIGN KEY(building_id) REFERENCES buildings(id) ON DELETE SET NULL, FOREIGN KEY(asset_id) REFERENCES assets(id) ON DELETE SET NULL);
        CREATE TABLE IF NOT EXISTS material_transactions (id INTEGER PRIMARY KEY AUTOINCREMENT, material_id INTEGER, transaction_type TEXT, quantity INTEGER, unit_cost INTEGER, reference TEXT, task_id INTEGER, warehouse_from TEXT, warehouse_to TEXT, date TEXT, notes TEXT, FOREIGN KEY(material_id) REFERENCES materials(id) ON DELETE CASCADE);
        CREATE TABLE IF NOT EXISTS purchases (id INTEGER PRIMARY KEY AUTOINCREMENT, po_no TEXT UNIQUE, item_name TEXT, category TEXT, quantity INTEGER, unit TEXT, price INTEGER, tax INTEGER DEFAULT 0, total_amount INTEGER, supplier TEXT, invoice_no TEXT, date TEXT, status TEXT DEFAULT 'مكتمل', notes TEXT);
        CREATE TABLE IF NOT EXISTS attendance (id INTEGER PRIMARY KEY AUTOINCREMENT, emp_id INTEGER, status TEXT, date TEXT, work_hours INTEGER DEFAULT 8, notes TEXT, UNIQUE(emp_id,date), FOREIGN KEY(emp_id) REFERENCES employees(id) ON DELETE CASCADE);
        CREATE TABLE IF NOT EXISTS finance (id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT, category TEXT, description TEXT, amount INTEGER, date TEXT, reference TEXT, task_id INTEGER, contract_id INTEGER);
        """)
...
if menu == "لوحة التحكم":
    ...
    with st.form("material"):
        a,b=st.columns(2)
        with a: code=st.text_input("كود الصنف *"); name=st.text_input("الاسم العربي *"); english=st.text_input("الاسم الإنجليزي"); category=st.selectbox("التصنيف",CATEGORIES); unit=st.selectbox("الوحدة",UNITS); qty=st.number_input("الرصيد الحالي", min_value=0, value=0, step=1); minimum=st.number_input("الحد الأدنى للمخزون", min_value=0, value=0, step=1)
        with b: reorder=st.number_input("نقطة إعادة الطلب", min_value=0, value=0, step=1); price=st.number_input("سعر التوريد", min_value=0, value=0, step=1); cost=st.number_input("متوسط التكلفة", min_value=0, value=0, step=1); supplier=st.text_input("المورد المعتمد"); storage=st.text_input("رقم المستودع والرف"); part=st.text_input("رقم القطعة البديلة (Part No)")
    ...
    with st.form("transaction"):
        selected=st.selectbox("الصنف أو قطعة الغيار",list(mm) or ["لا توجد مواد"]); typ=st.selectbox("نوع الحركة",["إضافة مشريات للمستودع","صرف قطع غيار لموقع عميل","مرتجع من موقع","تسوية زيادة","تسوية عجز"]); qty=st.number_input("الكمية", min_value=1, value=1, step=1); cost=st.number_input("سعر الوحدة", min_value=0, value=0, step=1); ref=st.text_input("مرجع أمر الصيانة أو رقم الإذن"); notes=st.text_area("ملاحظات الحركة")
        if st.form_submit_button("تنفيذ حركة المخزون",use_container_width=True) and mm:
            mid=mm[selected]; old=int(q("SELECT quantity FROM materials WHERE id=?",(mid,)).iloc[0,0]); new=old+qty if typ in ["إضافة مشريات للمستودع","مرتجع من موقع","تسوية زيادة"] else old-qty
    ...
    with st.form("purchase"):
        a,b=st.columns(2)
        with a: item=st.text_input("الصنف أو الخدمة المشتراة *"); category=st.selectbox("التصنيف",CATEGORIES); qty=st.number_input("الكمية", min_value=1, value=1, step=1); unit=st.selectbox("الوحدة",UNITS); price=st.number_input("سعر الوحدة", min_value=0, value=0, step=1)
        with b: tax=st.number_input("قيمة الضريبة", min_value=0, value=0, step=1); supplier=st.text_input("المورد الخارجي *"); invoice=st.text_input("رقم فاتورة المورد"); status=st.selectbox("حالة أمر الشراء",["مسودة","بانتظار الاعتماد المالي","قيد التوريد","مكتمل ومستلم","ملغي"])
    ...
    with st.form("employee"):
        ...
        with b: hire=st.date_input("تاريخ التعيين",date.today()); salary=st.number_input("الراتب الشهري", min_value=0, value=0, step=100); status=st.selectbox("الحالة",["على رأس العمل","إجازة سنوية","موقوف","منتهي الخدمة"]); skills=st.text_input("المهارات التخصصية أو شهادات الاعتماد")
    ...
    with st.form("attendance"):
        en=st.selectbox("الموظف",list(em) or ["لا يوجد موظفون"]); status=st.selectbox("حالة الدوام",["حاضر بموقع الشركة أو المشروع","غائب","إجازة","مرضي","مهمة صيانة خارجية","تأخير عن الموعد"]); d=st.date_input("تاريخ اليوم",date.today()); hours=st.number_input("ساعات العمل الفعلية", min_value=0, max_value=24, value=8, step=1); notes=st.text_input("ملاحظات الحضور")
    ...
    with st.form("finance"):
        ...
        with b: amount=st.number_input("المبلغ (ر.ق / ر.س)", min_value=0, value=0, step=1); d=st.date_input("تاريخ المعاملة",date.today()); ref=st.text_input("رقم الفاتورة أو المرجع المرتبط")
    ...
    rev=int(q("SELECT COALESCE(SUM(amount),0) n FROM finance WHERE type='إيراد'").iloc[0,0]); exp=int(q("SELECT COALESCE(SUM(amount),0) n FROM finance WHERE type='مصروف'").iloc[0,0]); c1,c2,c3=st.columns(3); c1.markdown(metric("إجمالي إيرادات العقود والخدمات",f"{rev:,} ر.ق"),unsafe_allow_html=True); c2.markdown(metric("إجمالي المصروفات التشغيلية",f"{exp:,} ر.ق"),unsafe_allow_html=True); c3.markdown(metric("صافي أرباح الشركة",f"{rev-exp:,} ر.ق"),unsafe_allow_html=True)
